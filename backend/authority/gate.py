...
class CommandRecord:
    ...
    def to_dict(self) -> dict:
        state_str = self.status.upper()
        sig = self.ledger_entry.get("signature") if self.ledger_entry else None
        return {
            "id": self.id,
            "command_id": self.id,
...
class AuthorityGate:
    ...
    def __init__(self, signer: CommandSigner | None = None):
        ...
        self._seed_initial_workflow()

    def _seed_initial_workflow(self) -> None:
        """Seed initial commands across all lifecycle stages for immediate testing."""
        # 1. Proposed Command (Ready to click Review)
        c1 = CommandRecord(
            command_type="EPS_SHED_NON_ESSENTIAL",
            payload={"mode": "LOAD_SHED", "target_subsystems": ["PAYLOAD_CAM", "HIGH_GAIN_RF"]},
            proposed_by="planner-langgraph",
            is_irreversible=False,
        )
        c1.id = "CMD-101"
        self._commands[c1.id] = c1

        # 2. Reviewed Command (Ready to click Verify)
        c2 = CommandRecord(
            command_type="ADCS_TARGET_SLEW",
            payload={"target_vector": [0.0, 0.0, 1.0], "slew_rate_limit_dps": 1.5},
            proposed_by="operator-deck-1",
            is_irreversible=False,
        )
        c2.id = "CMD-102"
        c2.status = "reviewed"
        c2.reviewed_by = "flight-ops-lead"
        c2.reviewed_at = datetime.now(timezone.utc)
        self._commands[c2.id] = c2

        # 3. Verified Command (Ready to click Seal & Approve)
        c3 = CommandRecord(
            command_type="SAFE_MODE_ENTER",
            payload={"reason": "CRITICAL_VOLTAGE_DROP", "shed_heaters": False},
            proposed_by="anomaly-detector",
            is_irreversible=True,
        )
        c3.id = "CMD-103"
        c3.status = "verified"
        c3.reviewed_by = "flight-ops-lead"
        c3.reviewed_at = datetime.now(timezone.utc)
        c3.verified_by = "safety-officer-gate"
        c3.verified_at = datetime.now(timezone.utc)
        self._commands[c3.id] = c3

        # 4. Approved & Sealed Command with HMAC in Ledger
        c4 = CommandRecord(
        ...
        c4.id = "CMD-091"
        c4.status = "approved"
        c4.reviewed_by = "ops-lead"
        c4.reviewed_at = datetime.now(timezone.utc)
        c4.verified_by = "safety-eng"
        c4.verified_at = datetime.now(timezone.utc)
        c4.approved_by = "director-flight"
        c4.approved_at = datetime.now(timezone.utc)

        payload_json = json.dumps(c4.payload, sort_keys=True)
        timestamp = c4.approved_at.isoformat()
        signature = self.signer.seal(payload_json, "director-flight", timestamp)
        ledger_entry = {
            "sequence_id": 1,
            "entry_id": "LEDGER-INIT-001",
            "command_id": c4.id,
            "command_type": c4.command_type,
            "payload_json": payload_json,
            "approver_id": "director-flight",
            "approved_by": "director-flight",
            "action": "approve",
            "timestamp": timestamp,
            "signature": signature,
            "previous_hash": "GENESIS_BLOCK_0000000000000000",
            "entry_hash": f"sha256_{signature[:32]}",
        ...
        c4.ledger_entry = ledger_entry
        self._commands[c4.id] = c4
