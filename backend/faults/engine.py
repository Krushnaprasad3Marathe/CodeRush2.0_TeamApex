...
class FaultEngine:
    ...
    def inject_fault(self, request: FaultInjectionRequest) -> ActiveFault:
        ...
        behavior = create_fault_behavior(request.fault_type.value, request.parameters)

        active_fault = ActiveFault(
            fault_id=fault_id,
            fault_type=request.fault_type.value,
            tier=request.tier.value,
            target_subsystem=request.target_subsystem.value,
        ...
        self._scorecard.append(ScorecardEntry(
            ...
            target_subsystem=request.target_subsystem.value,
            target_variable=request.target_variable,
            actual_root_cause=request.target_subsystem.value,
        ))

        logger.info(
            "Fault %s scheduled: %s %s on %s.%s at t=%d",
            fault_id, request.tier.value, request.fault_type.value,
            request.target_subsystem.value, request.target_variable,
            request.trigger_t,
        )
        return active_fault
