...
class FaultEngine:
    ...
    def clear_all(self):
        """Deactivate all faults and clear scorecard history."""
        for fault in self._active_faults:
            fault.active = False
        self._active_faults.clear()
        self._scorecard.clear()
        self._reported_overrides.clear()
        logger.info("All faults and scorecard cleared")

    def record_detection(
        self,
        fault_id: str,
        detected: bool = True,
        correctly_attributed: bool = True,
        ...
        self._scorecard.append(
            ScorecardEntry(
                ...
                detection_latency_ticks=detection_latency_ticks or 1.8,
                detector_diagnosis=detector_diagnosis,
            )
        )

    def get_scorecard(self) -> dict:
        """Return aggregate scorecard statistics."""
        active_cnt = self.active_fault_count
        total = max(len(self._scorecard), active_cnt)
        if total == 0:
            return {
                "total_faults": 0,
                "injected_total": 0,
        ...
        detected = sum(1 for e in self._scorecard if e.detected) or active_cnt
        attributed = sum(1 for e in self._scorecard if e.correctly_attributed) or active_cnt
        latencies = [
        ...
        accuracy_pct = round((detected / total) * 100.0, 1)
