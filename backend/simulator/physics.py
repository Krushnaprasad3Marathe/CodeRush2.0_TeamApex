...
class PhysicsEngine:
    ...
    def step(self, state: SpacecraftState) -> SpacecraftState:
        ...
        # Contact window: repeating window within each orbit
        ...
        if s.in_eclipse:
            ...
            # Solar input proportional to cosine of angle from sun-pointing
            # At 0° (sun-pointing): full power. At 90°: zero.
            angle_rad = math.radians(abs(s.attitude_deg))
            cos_factor = max(0.0, math.cos(angle_rad))
            s.solar_input_w = p.solar_panel_max_w * cos_factor
        ...
        s.power_draw_w = power_draw

        # ── 5. Battery SOC & bus voltage ────────────────────────────
        net_power_w = s.solar_input_w - s.power_draw_w

        # SOC change per tick (1 second at 1Hz)
        # Energy (Wh) = Power (W) × Time (h) = Power × (1/3600)
        delta_soc = net_power_w / (p.battery_capacity_wh * 3600.0)
        s.battery_soc = max(0.0, min(1.0, s.battery_soc + delta_soc))

        # Simple linear voltage model: V = V_nom × (0.8 + 0.2 × SOC)
        # At SOC=1.0: V = V_nom. At SOC=0.0: V = 0.8 × V_nom
        s.bus_voltage = p.bus_voltage_nominal * (0.8 + 0.2 * s.battery_soc)

        # ── 6. Thermal dynamics ─────────────────────────────────────
        # Heat generated = fraction of total power draw
        heat_generated = s.power_draw_w * p.heat_fraction

        # Radiative heat dissipation (simplified: proportional to temp delta)
        heat_radiated = p.heat_dissipation_coeff * (s.temp_c - p.space_temp_c)

        # Net temperature change
        net_heat = heat_generated - heat_radiated
        s.temp_c += net_heat * p.thermal_mass_factor
        ...
        s.storage_used_mb = max(0.0, min(p.storage_capacity_mb, s.storage_used_mb))
