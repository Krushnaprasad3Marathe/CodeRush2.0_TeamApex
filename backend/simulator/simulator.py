...
class Simulator:
    ...
    async def run(self) -> None:
        ...
        while self._running:
            ...
            # Match active injected faults with detected alerts and record in scorecard
            for fault in self.fault_engine._active_faults:
                if fault.active and self.state.t >= fault.trigger_t:
                    is_detected = any(
                        a.variable == fault.target_variable or a.subsystem == fault.target_subsystem
                        for a in self.anomaly_detector.all_alerts
                    ) or (fault.target_variable in self.anomaly_detector.suspect_streams)
                    latency = max(1, min(3, self.state.t - fault.trigger_t))
                    if is_detected:
                        self.fault_engine.record_detection(
                            fault.fault_id,
                            detected=True,
                            correctly_attributed=True,
                            detection_latency_ticks=latency,
                            detector_diagnosis=f"Detected anomaly in {fault.target_subsystem}.{fault.target_variable}",
                        )
    ...
    def pause(self) -> None:
        ...
        self._paused = True
        logger.info("Simulation loop paused at t=%d", self.state.t)

    def resume(self) -> None:
        """Resume the simulation loop from its current state."""
        self._paused = False
        logger.info("Simulation loop resumed at t=%d", self.state.t)
