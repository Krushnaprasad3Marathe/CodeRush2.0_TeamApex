...
@app.post("/ai/configure-key", tags=["ai"])
async def configure_gemini_key(body: ConfigureKeyRequest):
    """Configure or update the Gemini API key at runtime."""
    from ai.gemini_client import set_api_key
    success = set_api_key(body.api_key)
    return {
        "status": "configured" if success else "failed",
        "has_key": bool(body.api_key.strip()),
        "key_prefix": body.api_key[:6] + "..." if len(body.api_key) > 6 else "",
    }
...
async def get_ai_status():
    ...
    from ai.gemini_client import get_current_api_key
    key = get_current_api_key()
    return {
        "has_key": bool(key),
        "key_prefix": key[:6] + "..." if key and len(key) > 6 else None,
        "engine": "gemini-2.0-flash (Active)" if key else "template-fallback (Nominal)",
    }
...
async def ai_diagnose(body: DiagnoseRequest):
    ...
    try:
        ...
        explanation = await diagnose_anomaly(
            active_alerts, state_dict, diagnosis_dict, api_key=body.api_key
        )
    except Exception as e:
    ...
    procedures = []
    target_alert = active_alerts[0] if active_alerts else {"subsystem": "EPS (Power)", "description": "Nominal telemetry sweep", "severity": "info"}
    try:
        from ai.gemini_client import suggest_procedure
        procedures = await suggest_procedure(target_alert, state_dict, api_key=body.api_key)
    except Exception:
        pass
...
# ── Direct ZIP Code Download Endpoints ────────────────────────────────
@app.get("/download/frontend-zip", tags=["system"])
async def download_frontend_zip():
    """Download the complete frontend source code as a ZIP archive directly in browser."""
    from fastapi.responses import FileResponse
    possible_paths = [
        "aegis_mos_frontend_complete.zip",
        "../aegis_mos_frontend_complete.zip",
        "../../aegis_mos_frontend_complete.zip",
        os.path.join(os.getcwd(), "aegis_mos_frontend_complete.zip"),
        os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "aegis_mos_frontend_complete.zip"),
    ]
    for p in possible_paths:
        if os.path.exists(p) and os.path.getsize(p) > 1000:
            return FileResponse(p, filename="aegis_mos_frontend_complete.zip", media_type="application/zip")
    return {"status": "error", "message": "Frontend ZIP not found on server"}


@app.get("/download/project-zip", tags=["system"])
async def download_project_zip():
    """Download the full end-to-end project as a ZIP archive directly in browser."""
    from fastapi.responses import FileResponse
    possible_paths = [
        "aegis_mos_full_project.zip",
        "../aegis_mos_full_project.zip",
        "../../aegis_mos_full_project.zip",
        os.path.join(os.getcwd(), "aegis_mos_full_project.zip"),
        os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "aegis_mos_full_project.zip"),
    ]
    for p in possible_paths:
        if os.path.exists(p) and os.path.getsize(p) > 1000:
            return FileResponse(p, filename="aegis_mos_full_project.zip", media_type="application/zip")
    return {"status": "error", "message": "Full Project ZIP not found on server"}
